...
#' Similarly, in categorical features wilcox test is used to compare between the groups in the features based on their mutational count.
#' Scatter plot is used to represent the sequential features along with the R and p-value from the pearson correlation.
#' Similarly violin plots are used to plot the groups in the categorical data and wilcox or kruska-wallis values are shown on the graph.
#'
#'
...
feature.spot <-function(data, regions, feature){
  ...
  Sequential<-function (f)
  {
    ...
    plot<-suppressWarnings({ggplot2::ggplot(data=sample_df,aes(x=result,y=Count))+
  ...
  Categorical <-function (f)
  {
    ...
    for (cat in 1:(length(comb)-1))
    {
      ...
      wilcox_result<-suppressWarnings({wilcox.test(subset1,subset2)})
      print(wilcox_result)
    }
    plotc<- suppressWarnings({ggplot2::ggplot(sample_df,aes(x=result,y=Count, fill =result))+
  ...
  for (f in feature_list)
  {
    ...
    {
      plot_s<-Sequential(f)
      plot_list<-c(plot_list,list(plot_s))
    }
    else
    {
      plot_c<- Categorical(f)
      plot_list<-c(plot_list,list(plot_c))
    }
  }
  return(grid.arrange(grobs=plot_list,nrow=2))
